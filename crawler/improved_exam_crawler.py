...
class ImprovedExamCrawler:
    ...
    def parse_answer_table(self, html: str) -> Dict[int, str]:
        ...
        if not answers:
            ...
            # 匹配 "1 A 2 B 3 C" 格式
            pattern = re.compile(r'(\d+)\s+([A-D])')
            matches = pattern.findall(content[:5000])  # 只在开头查找答案表
            for num, ans in matches:
                if 1 <= int(num) <= 44:
                    answers[int(num)] = ans
...
def main():
    ...
    parser = argparse.ArgumentParser(description="改进版408真题爬虫")
    ...
    crawler = ImprovedExamCrawler()
    
    print("\n" + "=" * 60)
    print("408考研真题爬虫 - 完整版")
    print("=" * 60)
    print(f"\n爬取年份: {args.start_year} - {args.end_year}")
    print(f"输出格式: CSV {'✓' if not args.no_csv else '✗'} JSON {'✓' if not args.no_json else '✗'}\n")
